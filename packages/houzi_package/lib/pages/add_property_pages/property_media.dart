...
class PropertyMediaState extends State<PropertyMedia> {
  ...
  // fetchNonce() async {
  //   ApiResponse response = await _apiManager.fetchDeleteImageNonceResponse();
  //   if (response.success) {
  ...
  //                               "prop_id": propertyId,
  //                             };
  //                             final response = _apiManager.fetchDeleteImageFromEditProperty(deleteImageInfo, nonce);
  //                             if (response != null) {
...
class _AddPropertyMediaWidgetState extends State<AddPropertyMediaWidget> {
      ...
      child: Column(
        ...
        children: [
          ...
          LightButtonWidget(
              text: UtilityMethods.getLocalizedString("upload_media"),
              fontSize: AppThemePreferences.buttonFontSize,
              onPressed: ()=> _totalImages < (num.tryParse(MAX_PROP_IMAGES_UPLOAD) ?? 50)
                  ? chooseImageOption(context)
                  : _showToast(context)
          ),
          _imageMapsList.isNotEmpty
  ...
  Future getImageMultiple(BuildContext context) async {
    var pickedFiles = await picker.pickMultiImage(imageQuality: 90,maxHeight: 2000,maxWidth: 2000);
        previousLength = _imageMapsList.length;
    ...
    if (pickedFiles != null) {
      ...
      if(_imageMapsList.isNotEmpty && _imageMapsList.length <= (num.tryParse(MAX_PROP_IMAGES_UPLOAD) ?? 50)) {
        if (kDebugMode) {
          print("Total images in media widget after adding: ${_imageMapsList.length}");
        }
      } else {
        _imageMapsList.removeRange(previousLength, _imageMapsList.length);
       _showToast(context);
        return;
      }
      if(mounted) setState(() {
    ...
    return ReorderableGridView.count(
      onReorder: (oldIndex, newIndex) {
        setState(() {
          ...
          final item = _imageMapsList.removeAt(oldIndex);
          _imageMapsList.insert(newIndex, item);
        });
      },
                        ...
                        TextButtonWidget(
                          onPressed: () async {
                            ...
                            if(_imageStatus == UPLOADED){
                              ...
                              ApiResponse<String> response = await _apiManager.deleteImage(params, nonce);
