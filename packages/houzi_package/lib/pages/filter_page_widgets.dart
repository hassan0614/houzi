...
class _FilterPageWidgetsState extends State<FilterPageWidgets> {
  ...
  void initState() {
    ...
    if (_isMultiCurrencyEnabled) {
      _selectedCurrencySymbol = HiveStorageManager.readDefaultMultiCurrency();
    } else {
      _selectedCurrencySymbol =
          HiveStorageManager.readDefaultCurrencyInfoData() ?? "\$";
    }
    filterPageConfigElementMap = _apiManager.convertFilterPageElementToMap(widget.filterPageConfigData);

    Map<String, dynamic> data = HiveStorageManager.readCustomFieldsDataMaps();
    // print("BroadCasting from Filter_page_widget.dart: $data");
    if (data.isNotEmpty) {
      ApiManager _apiManager = ApiManager();
      Custom custom = _apiManager.getCustomFieldsData(data);
      _customFieldsList = custom.customFields ?? [];
      if(_customFieldsList.isNotEmpty){
      // print("BroadCasting from Filter_page_widget.dart for custom with parsing: $_customFieldsList");
        for(var customFieldItem in _customFieldsList){
          ...
          // print("Showing you the loop: $_customFieldsSectionTypeList");
        }
      }
    }
  ...
  bool isWidgetTypeRoomsStringPicker() {
    if (filterPageConfigElementMap[sectionTypeKey] == stringPickerKey &&
        filterPageConfigElementMap[dataTypeKey] == stringPickerRoomsKey) {
      return true;
    }
    return false;
  }
  ...
  void loadData() {
    ...
    if (_dataInitializationMap.isNotEmpty) {
      ...
      if (isWidgetTypeRoomsStringPicker()) {
        if (_dataInitializationMap.containsKey(ROOMS) &&
            _dataInitializationMap[ROOMS] != null &&
            _dataInitializationMap[ROOMS].isNotEmpty) {
          if(_dataInitializationMap[ROOMS] is List) {
            _selectedRoomsList = _dataInitializationMap[ROOMS];
          } else {
            _selectedRoomsList = [_dataInitializationMap[ROOMS]];
          }
        } else {
    ...
    switch(filterPageConfigElementMap[sectionTypeKey]) {
      ...
      default: {
        String tempSectionTypeKey =
            ...
            // print("Check now ${filterPageConfigElementMap[sectionTypeKey]}");
        if (_customFieldsSectionTypeList.isNotEmpty &&
            _customFieldsSectionTypeList.contains(tempSectionTypeKey)) {
              // print("Showing you the custom fields section type: ${widget.filterPageConfigData.sectionType}");
  ...
  Widget genericCustomFieldWidget({
    ...
    if(_customFieldsList.isNotEmpty) {
      for (var customFieldItem in _customFieldsList) {
        // print("this is here: ${_dataInitializationMap}");
        final transformMap = UtilityMethods.transformMapWithBracketKeys(_dataInitializationMap);
        // print("this is here: ${transformMap}, Check = ${transformMap.containsKey("custom_fields")}");

        if(customFieldItem.label == _tempCustomFieldLabel){
          return CustomFieldsWidget(
            fromFilterPage: true,
            customFieldData: customFieldItem,
            propertyInfoMap: transformMap.containsKey(PROPERTY_CUSTOM_FIELDS) ?
            transformMap[PROPERTY_CUSTOM_FIELDS] : {},
            customFieldsPageListener: (Map<String, dynamic> dataMap){
              if(dataMap.isNotEmpty){
                Map tempDataMap = {};
                // _dataInitializationMap.addAll(dataMap);
                if(transformMap.containsKey(PROPERTY_CUSTOM_FIELDS)){
                  tempDataMap.addAll(transformMap[PROPERTY_CUSTOM_FIELDS]);
                ...
                transformMap[PROPERTY_CUSTOM_FIELDS] = tempDataMap;
                widget.filterPageWidgetsListener(transformMap, "");
                if(mounted) setState(() {});
              }
            },
          );
  ...
  ResetFilterData(){
    if (mounted) {
      setState(() {
        ...
        if(isWidgetTypeRoomsStringPicker()){
          _selectedRoomsList = [];
          _dataInitializationMap.remove(ROOMS);
        }
        if(isWidgetTypeBedroomsStringPicker()){
          _selectedBedroomsList = [];
  ...
  IconData getStringPickerIconData(String dataType) {
    ...
    else if(dataType == stringPickerRoomsKey){
      return AppThemePreferences.roomIcon;
    }
  ...
  updateStringPickerData({
    ...
    if(dataType == stringPickerBedroomsKey){
      ...
      _selectedRoomsList = selectedItemsList;
      _dataInitializationMap[ROOMS] = _selectedRoomsList;
    }
