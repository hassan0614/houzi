...
class _AddPropertyRequestState extends State<AddPropertyRequest> with ValidationMixin {
  ...
  final _roomsTextController = TextEditingController();
  ...
  double stepperWidth = MediaQuery.of(context).size.width * 0.525;
    return Column(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        addNumberOfRooms(stepperWidth),
        addNumberOfBedrooms(stepperWidth),
        addNumberOfBathrooms(stepperWidth),
      ],
    );
  }

  Widget addNumberOfBedrooms(double givenWidth) {
  ...
  Widget addNumberOfRooms(double givenWidth) {
    return GenericStepperWidget(
      givenWidth: givenWidth,
      // padding: const EdgeInsets.fromLTRB(10, 20, 20, 0),
      labelText: UtilityMethods.getLocalizedString("rooms"),
      controller: _roomsTextController,
      textAlign: TextAlign.center,
      onRemovePressed: () {
        if (_rooms > 0) {
          setState(() {
            _rooms -= 1;
            _roomsTextController.text = _rooms.toString();
            addRequestPropertyInfoMap[INQUIRY_ROOMS] = _rooms.toString();
          });
        }
      },
      onAddPressed: () {
  ...
  Widget addNumberOfBathrooms(double givenWidth) {
    return GenericStepperWidget(
      givenWidth: givenWidth,
      // padding: const EdgeInsets.fromLTRB(10, 20, 20, 0),
