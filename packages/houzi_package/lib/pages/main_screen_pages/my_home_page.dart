...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void refreshAllPageList() {
    final int currentIndex = _selectedIndex;

    /// get all tabs to LoadingPage to free up cached states
    setState(() {
      for (int i = 0; i < pageList.length; i++) {
        pageList[i] = LoadingPage(
          key: ObjectKey(random.nextInt(100000)),
          showAppBar: false,
        );
      }
    });

    /// Rebuild the navbar and pages from scratch
    Future.microtask(() {
      if (!mounted) return;
      setState(() {
        pageList.clear();
        bottomNavBarItemsMap.clear();
        homePageIndex = -1;
        searchPageIndex = -1;
        savedOrFavSearchesPageIndex = -1;
        setUpBottomNavbar();
        _selectedIndex = currentIndex < pageList.length ? currentIndex : 0;
      });
    });
  }
  @override
