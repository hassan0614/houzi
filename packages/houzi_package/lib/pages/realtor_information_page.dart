    ...
    if (realtorInformation.containsKey(AGENCY_DATA)) {
      setState(() {
        ...
        if (item != null) {
          ...
          title = item.title ?? "";
          verificationTag = item.agencyVerification ?? false;
      ...
      setState(() {
        ...
        if (item != null) {
          ...
          image = item.thumbnail ?? "";
          title = item.title ?? "";
          verificationTag = item.agentVerification ?? false;
          content = item.content ?? "";
          if (content.isNotEmpty) {
            content = UtilityMethods.stripHtmlIfNeeded(content);
          }
      ...
      setState(() {
        ...
        if (item != null) {
          ...
          verificationTag = item[tempRealtorVerificationKey] ?? false;
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    RealtorInfoWidget(
                      title: title ?? "",
                      isAgentVerified: verificationTag,
                      imageUrl: image,
              ...
              Row(
                children: [
                  ...
                  Padding(
                      ...
                      child: isAgentVerified
                          ? 
                          // AppThemePreferences()
                          //     .appTheme
                          //     .agentAgencyVerifiedIcon
RealtorVerificationWidget(context)                          : null),
                ],
              ),
              AgentPositionWidget(
                company: company,
...
Widget RealtorVerificationWidget(BuildContext context) {
  final textDirection = Directionality.of(context);

  return Container(
    decoration: BoxDecoration(
      color: AppThemePreferences.realtorVerificationColor,
      borderRadius: BorderRadius.circular(15.0),
    ),
    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), // Moved here
    child: Row(
      ...
      children: [
        GenericTextWidget(
          UtilityMethods.getLocalizedString("realtor_verified"),
          ...
          style: AppThemePreferences().appTheme.label04TextStyle?.copyWith(
                color: AppThemePreferences.realtorverificationTagColor,
                fontWeight: FontWeight.w600,
                fontSize: 9.0,
              ),
        ),
        const SizedBox(width: 5), // spacing between text and icon
        if (AppThemePreferences().appTheme.realtorVerificatinIcon != null)
          AppThemePreferences().appTheme.realtorVerificatinIcon!,
      ],
    ),
  );
}
                                            ...
                                            child: InkWell(
                                              ...
                                              const BorderRadius.all(
                                                  Radius.circular(
                                                      10)),
                                              onTap: () {
                                                print("Check which is selected realtor type: $isAgent, $isAgency");
                                                Navigator.push(
                                                  context,
                                                  MaterialPageRoute(
                                                    builder: (context) =>
