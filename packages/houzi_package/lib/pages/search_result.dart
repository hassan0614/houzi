...
class SearchResultState extends State<SearchResult> {
  ...
  int? _totalResults;
  int page = 1;
  int perPage = SearchPropertiesPerPageListings;
  ...
  Map<String, dynamic> mapFromFilterScreen = {}; //map that we get from filter screen.
  Map<String, dynamic> _searchApiMap = {}; // map that is sent to search api
  Map<String, dynamic> _chipsShadowMap = {}; // A shadow map that has almost all keys to navigate to Filter Screen from top chips.
  List _filterChipsDisplayList = []; //List of map, to show chips below the top search bar. each item in this list belong to a chip.
  ...
  double slidingPanelPosition = 1.0;


  @override
  void initState() {
    super.initState();
    // print("SearchResultState::initState() called: $_chipsShadowMap");
              ...
              body: hasInternet == false
                  ...
                  : Stack(alignment: Alignment.topCenter, children: <Widget>[
                      SlidingPanelWidget(
                          ...
                          if(mounted) {
                            setState(() {
                              ...
                              if (sliderPosition != null ) {
                                slidingPanelPosition = sliderPosition;
                              ...
                              if(coordinatesMap != null && coordinatesMap.isNotEmpty){
                                if(mounted) {
                                  setState(() {
                                    ...
                                    _searchApiMap[SEARCH_LOCATION] = "true";
                                    _searchApiMap[USE_RADIUS] = "on";
                                    _searchApiMap.addAll(coordinatesMap);
                                    _chipsShadowMap.addAll(coordinatesMap);
                        ...
                        panelBuilder: (panelScrollController) {
                              ...
                              if(!_isAtBottom){
                                ...
                                if(!_infiniteStop && _isPaginationFree){
                                  ...
                                  setState(() {
                                    ...
                                    page += 1;
                                    _searchApiMap[SEARCH_RESULTS_CURRENT_PAGE] = '$page';
                                    _futureFilteredArticles =
                                        fetchFilteredArticles(page, perPage, _searchApiMap, fetchSubListing: widget.fetchSubListing);
                                  });
                                  // print("Page: $page........");
                                  _isAtBottom = false;
                                }
                          ...
                          return SearchResultBuilderWidget(
                            ...
                            slidingPanelPosition:slidingPanelPosition,
                      ...
                      SearchResultsSearchBarWidget(
                        ...
                        searchApiMap: _searchApiMap,
                        chipsSearchDataMap: _chipsShadowMap,
                        filterChipsDisplayList: _filterChipsDisplayList,
  ...
  void doSearch() {
    ...
    if (isAuthor) {
      ...
      if (mapFromFilterScreen.isNotEmpty) {
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, ROOMS)) {
          String tempRoomsString = '';
          List<String> tempRoomsStringList = [];
          if (mapFromFilterScreen[ROOMS] is List) {
            tempRoomsStringList = List<String>.from(
                mapFromFilterScreen[ROOMS].map((e) => e.toString()));
          } else {
            tempRoomsStringList = [mapFromFilterScreen[ROOMS].toString()];
          }

          if (tempRoomsStringList.isNotEmpty) {
            tempRoomsStringList = UtilityMethods.getSanitizedStringPickerList(
                tempRoomsStringList);
            tempRoomsString = tempRoomsStringList.join(',');
          }
          _searchApiMap[SEARCH_RESULTS_ROOMS] = tempRoomsString;
          _filterChipsDisplayList.add({
            ROOMS: tempRoomsString,
          });

          List<String> chipsSearchDataList = tempRoomsString.split(",");
          ...
          _chipsShadowMap[ROOMS] = chipsSearchDataList;
        }
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, BEDROOMS)) {
          ...
          if (mapFromFilterScreen[BEDROOMS] is List) {
            tempBedroomStringList = List<String>.from(
                mapFromFilterScreen[BEDROOMS].map((e) => e.toString()));
          } else {
            tempBedroomStringList = [mapFromFilterScreen[BEDROOMS].toString()];
          }
          ...
          _searchApiMap[SEARCH_RESULTS_BEDROOMS] = tempBedroomString;
          _filterChipsDisplayList.add({
          ...
          _chipsShadowMap[BEDROOMS] = chipsSearchDataList;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, BATHROOMS)) {
          ...
          if (mapFromFilterScreen[BATHROOMS] is List) {
            tempBathroomStringList = List<String>.from(
                mapFromFilterScreen[BATHROOMS].map((e) => e.toString()));
          } else {
            tempBathroomStringList = [
              mapFromFilterScreen[BATHROOMS].toString()
            ];
          }
          ...
          _searchApiMap[SEARCH_RESULTS_BATHROOMS] = tempBathroomString;
          _filterChipsDisplayList.add({
          ...
          _chipsShadowMap[BATHROOMS] = chipsSearchDataList;
        }

        if (_searchApiMap.containsKey(SEARCH_RESULTS_BEDROOMS) ||
            _searchApiMap.containsKey(SEARCH_RESULTS_ROOMS) ||
            _searchApiMap.containsKey(SEARCH_RESULTS_BATHROOMS)) {
          _searchApiMap[SEARCH_RESULTS_BEDS_BATHS_CRITERIA] = "IN";
        }

        _searchApiMap.remove(SEARCH_RESULTS_STATUS);
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_STATUS_SLUG)) {
          ...
          _searchApiMap[SEARCH_RESULTS_STATUS] = nonEmpty;
          _chipsShadowMap[PROPERTY_STATUS_SLUG] = nonEmpty;
        }
        else {
          _chipsShadowMap[PROPERTY_STATUS_SLUG] = ["all"];
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_STATUS)) {
          ...
          _chipsShadowMap[PROPERTY_STATUS] = nonEmpty;
        }
        else {
          _filterChipsDisplayList.add({
            PROPERTY_STATUS: ["All"],
          });
          _chipsShadowMap[PROPERTY_STATUS] = ["All"];
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_TYPE)) {
          ...
          _chipsShadowMap[PROPERTY_TYPE] = tempPropertyTypeList;
          _chipsShadowMap[PROPERTY_TYPE_SLUG] = tempPropertyTypeSlugsList;
          ...
          _chipsShadowMap[PROPERTY_TYPE] = ["All"];
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_LABEL)) {
          ...
          _chipsShadowMap[PROPERTY_LABEL] = nonEmpty;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_FEATURES)) {
          ...
          _chipsShadowMap[PROPERTY_FEATURES] = nonEmpty;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_TYPE_SLUG)) {
          ...
          _searchApiMap[SEARCH_RESULTS_TYPE] = nonEmpty;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_LABEL_SLUG)) {
          ...
          _searchApiMap[SEARCH_RESULTS_LABEL] = nonEmpty;
          _chipsShadowMap[PROPERTY_LABEL_SLUG] = mapFromFilterScreen[PROPERTY_LABEL_SLUG];
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, CITY_SLUG)) {
          ...
          _chipsShadowMap[CITY_SLUG] = mapFromFilterScreen[CITY_SLUG];
          ...
          if (nonEmpty.isNotEmpty) {
            _searchApiMap[SEARCH_RESULTS_LOCATION] = nonEmpty;
          }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_AREA_SLUG)) {
          ...
          element.isNotEmpty && element != "all").toList();
          if (nonEmpty.isNotEmpty) {
            _searchApiMap[SEARCH_RESULTS_AREA] = nonEmpty;
          }
          _chipsShadowMap[PROPERTY_AREA_SLUG] = nonEmpty;
        }


        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_KEYWORD)) {
          _searchApiMap[SEARCH_RESULTS_KEYWORD] = mapFromFilterScreen[PROPERTY_KEYWORD];
          _filterChipsDisplayList.add({
            PROPERTY_KEYWORD: mapFromFilterScreen[PROPERTY_KEYWORD],
          });

          _chipsShadowMap[PROPERTY_KEYWORD] = mapFromFilterScreen[PROPERTY_KEYWORD];
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_COUNTRY_SLUG)) {
          ...
          element.isNotEmpty && element != "all").toList();
          if (nonEmpty.isNotEmpty) {
            _searchApiMap[SEARCH_RESULTS_COUNTRY] = nonEmpty;
          }
          _chipsShadowMap[PROPERTY_COUNTRY_SLUG] = nonEmpty;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_STATE_SLUG)) {
          ...
          element.isNotEmpty && element != "all").toList();
          if (nonEmpty.isNotEmpty) {
            _searchApiMap[SEARCH_RESULTS_STATE] = nonEmpty;
          }
          _chipsShadowMap[PROPERTY_STATE_SLUG] = nonEmpty;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_FEATURES_SLUG)) {
          ...
          element.isNotEmpty && element != "all").toList();
          _searchApiMap[SEARCH_RESULTS_FEATURES] = nonEmpty;
          _chipsShadowMap[PROPERTY_FEATURES_SLUG] = nonEmpty;
        }


        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, AREA_MAX)) {
          _searchApiMap[SEARCH_RESULTS_MAX_AREA] = mapFromFilterScreen[AREA_MAX];
          _chipsShadowMap[AREA_MAX] = mapFromFilterScreen[AREA_MAX];
          // filterChipsRelatedList.add({
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, AREA_MIN)) {
          _searchApiMap[SEARCH_RESULTS_MIN_AREA] =
          mapFromFilterScreen[AREA_MIN];
          _chipsShadowMap[AREA_MIN] = mapFromFilterScreen[AREA_MIN];
          // filterChipsRelatedList.add({
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PRICE_MIN)) {
          _searchApiMap[SEARCH_RESULTS_MIN_PRICE] =
          mapFromFilterScreen[PRICE_MIN];
          _chipsShadowMap[PRICE_MIN] = mapFromFilterScreen[PRICE_MIN];
          // filterChipsRelatedList.add({
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PRICE_MAX)) {
          _searchApiMap[SEARCH_RESULTS_MAX_PRICE] =
          mapFromFilterScreen[PRICE_MAX];
          _chipsShadowMap[PRICE_MAX] = mapFromFilterScreen[PRICE_MAX];
          // filterChipsRelatedList.add({
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, LATITUDE)) {
          _chipsShadowMap[LATITUDE] = mapFromFilterScreen[LATITUDE];
          _searchApiMap[LATITUDE] = mapFromFilterScreen[LATITUDE];
        }


        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, RADIUS)) {
          _chipsShadowMap[RADIUS] = mapFromFilterScreen[RADIUS];
          _searchApiMap[RADIUS] = mapFromFilterScreen[RADIUS];
        }


        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, LONGITUDE)) {
          _chipsShadowMap[LONGITUDE] = mapFromFilterScreen[LONGITUDE];
          _searchApiMap[LONGITUDE] = mapFromFilterScreen[LONGITUDE];
        }


        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, USE_RADIUS)) {
          _chipsShadowMap[USE_RADIUS] = mapFromFilterScreen[USE_RADIUS];
          _chipsShadowMap[SEARCH_LOCATION] = mapFromFilterScreen[SEARCH_LOCATION];
          var temp = mapFromFilterScreen[USE_RADIUS];
          if (temp == "on") {
            _searchApiMap[USE_RADIUS] = mapFromFilterScreen[USE_RADIUS];
            _searchApiMap[SEARCH_LOCATION] = mapFromFilterScreen[SEARCH_LOCATION];
          }
        }

        // print("The data that is usng here : $mapFromFilterScreen , tHE DATA inside the hive: ${HiveStorageManager.readFilterDataInfo() ?? {}}, Status = ${UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_CUSTOM_FIELDS)}",);
        final transformMap = UtilityMethods.transformMapWithBracketKeys(mapFromFilterScreen);
        // print("The data that is usng here : $transformMap , tHE DATA inside the hive: ${HiveStorageManager.readFilterDataInfo() ?? {}}, Status = ${UtilityMethods.isMapItemPopulated(transformMap, PROPERTY_CUSTOM_FIELDS)}",);

       if (UtilityMethods.isMapItemPopulated(transformMap, PROPERTY_CUSTOM_FIELDS)) {
            Map tempMap = transformMap[PROPERTY_CUSTOM_FIELDS];
            _chipsShadowMap[PROPERTY_CUSTOM_FIELDS] = transformMap[PROPERTY_CUSTOM_FIELDS];

            tempMap.forEach((key, value) {
            String dataKey = "$PROPERTY_CUSTOM_FIELDS[$key]";
            if (value is Map) {
              List tempList = [];
            ...
            _searchApiMap[tempDataKey] = tempList;
            _filterChipsDisplayList.add({
            tempDataKey: tempList,
            });
            } else {
            _searchApiMap[dataKey] = value;
            _filterChipsDisplayList.add({
            dataKey: [value], 
            });
          }
        ...
        if (UtilityMethods.getBooleanItemValueFromMap(inputMap: mapFromFilterScreen, key: showFeaturedKey)) {
          _searchApiMap[SEARCH_RESULTS_FEATURED] = "1";
          _filterChipsDisplayList.add({
            FEATURED_CHIP_KEY : UtilityMethods.getLocalizedString(FEATURED_CHIP_VALUE),
          });

          _chipsShadowMap[showFeaturedKey] = true;
        }

        _filterChipsDisplayList.remove(PROPERTY_COUNTRY);
        _filterChipsDisplayList.remove(PROPERTY_STATE);
        _filterChipsDisplayList.remove(CITY);
        _filterChipsDisplayList.remove(PROPERTY_AREA);

        _chipsShadowMap.remove(PROPERTY_COUNTRY);
        _chipsShadowMap.remove(PROPERTY_STATE);
        _chipsShadowMap.remove(CITY);
        _chipsShadowMap.remove(PROPERTY_AREA);
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_AREA)) {
          ...
          _chipsShadowMap[PROPERTY_AREA] = nonEmpty;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, CITY)) {
          ...
          _chipsShadowMap[CITY] = mapFromFilterScreen[CITY];
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_STATE)) {
          ...
          _chipsShadowMap[PROPERTY_STATE] = nonEmpty;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, PROPERTY_COUNTRY)) {
          ...
          _chipsShadowMap[PROPERTY_COUNTRY] = nonEmpty;
        }
        ...
        if (_searchApiMap.containsKey(SEARCH_RESULTS_AREA)) {
          //remove city, state and country
          _searchApiMap.remove(SEARCH_RESULTS_LOCATION);
          _searchApiMap.remove(SEARCH_RESULTS_STATE);
          _searchApiMap.remove(SEARCH_RESULTS_COUNTRY);
        }

        if (_searchApiMap.containsKey(SEARCH_RESULTS_LOCATION)) {
          //remove state and country
          _searchApiMap.remove(SEARCH_RESULTS_STATE);
          _searchApiMap.remove(SEARCH_RESULTS_COUNTRY);
        }

        if (_searchApiMap.containsKey(SEARCH_RESULTS_STATE)) {
          //remove country
          _searchApiMap.remove(SEARCH_RESULTS_COUNTRY);
        }
        ...
        if(mapFromFilterScreen.containsKey(REALTOR_SEARCH_TYPE)) {
          ...
          if (mapFromFilterScreen[REALTOR_SEARCH_TYPE] == REALTOR_SEARCH_TYPE_AGENT) {
            _searchApiMap[REALTOR_SEARCH_AGENT] = mapFromFilterScreen[REALTOR_SEARCH_ID];
          } else if (mapFromFilterScreen[REALTOR_SEARCH_TYPE] == REALTOR_SEARCH_TYPE_AGENCY) {
            _searchApiMap[REALTOR_SEARCH_AGENCY] = mapFromFilterScreen[REALTOR_SEARCH_ID];
          }
          _chipsShadowMap [REALTOR_SEARCH_NAME] = mapFromFilterScreen[REALTOR_SEARCH_NAME];
          _chipsShadowMap [REALTOR_SEARCH_TYPE] = mapFromFilterScreen[REALTOR_SEARCH_TYPE];
          _chipsShadowMap [REALTOR_SEARCH_ID] = mapFromFilterScreen[REALTOR_SEARCH_ID];
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, metaKeyFiltersKey)) {
          ...
          _chipsShadowMap[metaKeyFiltersKey] = _metaKeyFiltersMap;
          ...
          for (String key in _mapKeys) {
            ...
            if ( _pickerType == stringPickerKey
              ...
              _filterChipsDisplayList.add({key : _valueJson});
              ...
              _filterChipsDisplayList.add({key : _valueStr});
          ...
          _searchApiMap[metaKeyFiltersKey] = _metaKeyFiltersQueryMapJson;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(mapFromFilterScreen, keywordFiltersKey)) {
          ...
          _chipsShadowMap[keywordFiltersKey] = _keywordFiltersMap;
          ...
          _searchApiMap[keywordFiltersKey] = _keywordFiltersQueryMapJson;
        }
        ...
        if (UtilityMethods.isMapItemPopulated(
          ...
          _searchApiMap[PROPERTY_COUNTRY_QUERY_TYPE] =
              mapFromFilterScreen[PROPERTY_COUNTRY_QUERY_TYPE];
        }

        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_STATE_QUERY_TYPE)) {
          _searchApiMap[PROPERTY_STATE_QUERY_TYPE] =
          mapFromFilterScreen[PROPERTY_STATE_QUERY_TYPE];
        }

        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_AREA_QUERY_TYPE)) {
          _searchApiMap[PROPERTY_AREA_QUERY_TYPE] =
          mapFromFilterScreen[PROPERTY_AREA_QUERY_TYPE];
        }

        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_STATUS_QUERY_TYPE)) {
          _searchApiMap[PROPERTY_STATUS_QUERY_TYPE] =
          mapFromFilterScreen[PROPERTY_STATUS_QUERY_TYPE];
        }

        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_TYPE_QUERY_TYPE)) {
          _searchApiMap[PROPERTY_TYPE_QUERY_TYPE] =
          mapFromFilterScreen[PROPERTY_TYPE_QUERY_TYPE];
        }

        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_LABEL_QUERY_TYPE)) {
          _searchApiMap[PROPERTY_LABEL_QUERY_TYPE] =
          mapFromFilterScreen[PROPERTY_LABEL_QUERY_TYPE];
        }

        if (UtilityMethods.isMapItemPopulated(
            mapFromFilterScreen, PROPERTY_FEATURES_QUERY_TYPE)) {
          _searchApiMap[PROPERTY_FEATURES_QUERY_TYPE] =
          mapFromFilterScreen[PROPERTY_FEATURES_QUERY_TYPE];
        }

        _searchApiMap[SEARCH_RESULTS_CURRENT_PAGE] = '$page';

        _searchApiMap[SEARCH_RESULTS_PER_PAGE] = '${perPage}';
      }
    ...
    // print("SearchResults filteredMapForSearch: $_searchApiMap");
  ...
  {
    dataMap[PageKey] = "$page";
    dataMap[PerPageKey] = "$perPage";
    int count = 0;
    ...
    if (fetchSubListing) {
      ...
      ApiResponse<List> response =
          await _apiManager.fetchMultipleArticles(widget.subListingIds);
      internetAvailable = response.internet;
    ...
    Map<String,dynamic> tempFilterDataMap = _searchApiMap; //HiveStorageManager.readFilterDataInfo() ?? {};
