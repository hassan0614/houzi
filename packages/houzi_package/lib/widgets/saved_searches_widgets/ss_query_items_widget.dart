...
class QueryFeaturesDetailsWidget extends StatelessWidget {
      ...
      children: [
        Row(
          ...
          children: [
            SavedSearchQueryDataItemRowWidget(
              label: getRoomsLabel(),
              iconData: AppThemePreferences.roomIcon,
  ...
  String getRoomsLabel() {

    if ( queryDataMap[ROOMS] != null ) {
      return queryDataMap[ROOMS].last;
    }
      ...
      if ( _metaKeyFiltersMap[favPropertyRoomsKey] is Map ) {

        List _roomsValuesList = [];
        Map _roomsMetaMap = _metaKeyFiltersMap[favPropertyRoomsKey];

        if ( _roomsMetaMap[metaValueKey] is String ) {
          String _bedroomsValuesStr = _roomsMetaMap[metaValueKey];
          _roomsValuesList = _bedroomsValuesStr.split(",");
          return _roomsValuesList.last;
        }

        else if ( _roomsMetaMap[metaValueKey] is List ) {
          _roomsValuesList = _roomsMetaMap[metaValueKey];
          return _roomsValuesList.last;
        }
      }
  ...
  String getMaxPriceLabel() {
    ...
    if ( queryDataMap[PRICE_MAX] != null
      ...
      return UtilityMethods.makePriceCompact(queryDataMap[PRICE_MAX], "", queryDataMap[CURRENCY_SYMBOL]);
    }
      ...
      if ( _metaKeyFiltersMap[favPropertyPriceKey] is Map ) {
       ...
       return UtilityMethods.makePriceCompact(_metaPriceValue, "", queryDataMap[CURRENCY_SYMBOL]);
      }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Icon(
        AppThemePreferences.dotIcon,
        size: AppThemePreferences.dotIconSize,
        color: AppThemePreferences.appSecondaryColor,
      ),
    );
  }
}
